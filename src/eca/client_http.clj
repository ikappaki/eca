(ns eca.client-http
  ...
  (:import [eca AuthShim]
           [java.net URI URL]
           [java.net Authenticator$RequestorType PasswordAuthentication
            Proxy Proxy$Type InetSocketAddress ProxySelector]
           [java.io IOException]))
...
(defn hato-client-make
  ...
  It also supports optional specialized keys `:eca.client-http/proxy-http`
  and `:eca.client-http/proxy-https`. These maps specify proxy servers that
  the Hato client should use for outgoing HTTP and HTTPS requests.
        ...
        proxy-selector (when (or proxy-http-addr proxy-https-addr)
                         (proxy [ProxySelector] []
                           ...
                           (connectFailed [_ _ ^IOException e]
                             (.printStackTrace e))))
        authenticator (when (or (and http-user http-pass)
                                (and https-user https-pass))
                        (proxy [AuthShim] ;; Auth
                               []
                          (getPasswordAuthentication []
                            (let [^eca.AuthShim this this]
                              (when (= (.reqType this) Authenticator$RequestorType/PROXY)
                                (let [^URL url (.reqURL this)
                                      scheme (.getProtocol url)]
                                  (case scheme
                                    "http"  (when  (and http-user http-pass)
                                              (PasswordAuthentication. http-user
                                                                       (char-array http-pass)))
                                    "https" (when (and https-user https-pass)
                                              (PasswordAuthentication. https-user
                                                                       (char-array https-pass)))
                                    nil)))))))]
    (cond-> opts
      proxy-selector
